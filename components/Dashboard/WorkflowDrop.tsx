...
interface WorkflowDropProps {
    workflows: Workflow[]
}


export default function WorkflowDrop(props: WorkflowDropProps) {
    return (
        <div className="relative inline-flex w-56 mx-3">
            ...
            <select className="w-56 border border-gray-300 rounded-full text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none">
                {props.workflows.map(workflow => (
                    <DropItem nome={workflow.nome} />
                ))}
            </select>
        </div>
    )
